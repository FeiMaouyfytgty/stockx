...
$('.ditailed>div').click(function () {
  $(this).addClass('active').siblings().removeClass('active'); // console.log($(this).text());

  if ($(this).text() == '注册') {
    $('.login-lon').css('display', 'none');
    $('.reg-reg').css('display', 'block'); //判断登录注册按钮颜色改变
    ...
    $('.login-lon').css('display', 'block'); //判断登录注册按钮颜色改变
...
$('input').focus(function () {
  $(this).prev("label").stop().animate({
    ...
    "top": "3px",
    "left": '10px'
  }, 300, 'linear'); // $(this).children('input').focus();
}).hover(function () {
  $(this).attr('style', 'cursor: text;');
});
...
}).hover(function () {// $(this).next().attr('style','cursor: text;')
});
$("input").blur(function () {
  if ($(this).val() == '') {
    $(this).prev("label").stop().animate({
